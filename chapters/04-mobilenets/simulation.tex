...
In Figure~\ref{c4:fig:virtualized_instanceuse} the \gls{CDF} of the number of active instances for four different virtual \gls{GGSN} configurations is displayed. We observe, that increasing the number of supported tunnels per server allows a larger percentage of instances to be shutdown or used for other tasks. This demonstrates the scaling capability of the virtualized model quite well. Note, that both the scenario with \numprint{30} instances and \numprint{150} maximum tunnels per server as well as the scenario with \numprint{60} instances and \numprint{75} maximum tunnels per server share the same maximum amount of tunnels, \numprint{4500}, being right at the center of the interesting range of candidates.
