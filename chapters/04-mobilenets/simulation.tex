...
To get into more detail, Figure~\ref{c4:fig:virtualized_instanceuse} displays the \gls{CDF} of the portion of time a specific number of instances was active. Depicted are four configurations differing in the number of total instances and their tunnel capacity.
Increasing the number of the individual instances' tunnel capacity allows a larger percentage of instances to be shutdown or used for other tasks. This demonstrates the scaling capability of the virtualized model quite well. Note, that both the scenario with \numprint{30} instances and \numprint{150} maximum tunnels per server as well as the scenario with \numprint{60} instances and \numprint{75} maximum tunnels per server share the same maximum amount of tunnels, \numprint{4500}, being right at the center of the interesting range of candidates.
