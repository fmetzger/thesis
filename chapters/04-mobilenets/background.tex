...
Conceptually, \gls{gtp} is structured through a base packet header, a number of extension headers and a message body consisting of a series of \gls{IE}. The base header is depicted in Figure~\ref{c4:fig:gtpheader} and has a total length of \SI{12}{\byte}. Essential to the header are the \gls{TEID} to identify the corresponding userplane tunnel and the \SI{8}{\bit} message type field. Each of the message types corresponds to a specific signaling interaction from the overarching control plane procedures.
...
\begin{table}[htb]
\caption{All \gls{IE} in a Create \gls{PDP} Context request and size thereof, excluding the first message type Byte.}
\label{c4:tab:createrequestelements}
	\begin{tabu} to 0.5\textwidth{|X[3]|X[1]|X[1]|} \hline
		\textbf{\gls{IE}} & \textbf{Presence} & \textbf{Typical Size}\\ \hline
		\gls{IMSI} & cond. & 8 \\ \hline
		\acrshort{RAI} & cond. & 6 \\ \hline
		Recovery & opt. & 1 \\ \hline
		Selection mode	& cond. & 1 \\ \hline
		\gls{TEID} Data I & mand. & 4 \\ \hline
		\gls{TEID} Control Plane & cond. & 4 \\ \hline
		\gls{NSAPI} & mand. & 1 \\ \hline
		Linked \gls{NSAPI} & cond. & 1 \\ \hline
		Charging Characteristics & cond. & 2 \\ \hline
		Trace Reference & opt. & 3\\ \hline
		Trace Type & opt. & 3\\ \hline
		End User Address & cond. & 9 (for IPv4)\\ \hline
		\gls{APN} & cond. & 3 + Name\\ \hline
		\acrshort{PCO} & opt. & min. 3\\ \hline
		\gls{SGSN} signaling address & mand.  & 9 (for IPv4)\\ \hline
		\gls{SGSN} user traffic address & mand. & 9 (for IPv4)\\ \hline
		\gls{MSISDN} & cond. & 3 + MSISDN \\ \hline
		\gls{QoS} Profile & mand. & min. 5\\ \hline
		\gls{TFT} & cond. & min. 4\\ \hline
	\end{tabu}%
	\begin{tabu} to 0.5\textwidth{|X[3]|X[1]|X[1]|} \hline
		\textbf{\gls{IE}} & \textbf{Presence} & \textbf{Typical Size} \\ \hline
		 Trigger Id & opt. & min. 4 \\ \hline
		 \acrshort{OMC} Identity & opt. & min. 4 \\ \hline
		 Common Flags & opt. & 4\\ \hline
		 \gls{APN} Restriction & opt. & 4\\ \hline
		 \gls{RAT} & opt. & 4\\ \hline
		 User Location Information & opt. & 11\\ \hline
		 MS Time Zone & opt. & 5\\ \hline
		 \gls{IMEI}(\acrshort{SV}) & cond. & 11\\ \hline
		 \gls{CAMEL} Charging Information Container & opt. & min. 4\\ \hline
		 Additional Trace Info & opt. & 12\\ \hline
		 Correlation-ID & opt. & 4\\ \hline
		 Evolved Allocation Retention Priority I & opt. & 4\\ \hline
		 Extended Common Flags & opt. & min. 4 \\ \hline
		 User \acrshort{CSG} Information & opt. & 11 \\ \hline
		 \gls{APN}-\acrshort{AMBR} & opt.  & 11 \\ \hline
		 Signaling Priority Indication & opt. & min. 4\\ \hline
		 Max \acrshort{MBR}/\acrshort{APN}-\acrshort{AMBR} & opt. & ?min. 11 \\ \hline
		 Private Extension & opt. & ?min 6\\ \hline
	\end{tabu}
\end{table}
