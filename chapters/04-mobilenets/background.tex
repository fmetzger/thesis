...
Short description of general node tasks in the \gls{PS} domain
\begin{itemize}
	\item \gls{SGSN}:
	\item \gls{GGSN}:
...
%	\item 23.002 \cite{3gpp.23.002} Network Architecture : general arch overview, node and interface description
	...
	\item 23.101 \cite{3gpp.23.101} general \gls{UMTS} architecture, focus on \gls{UE} / \gls{USIM} and strata
	\item 
	\item 24.008 \cite{3gpp.24.008} general GSM/UMTS core network signaling protocols and procedures
	\item 25.201 \cite{3gpp.25.201} \gls{UTRAN} physical layer description; channels
	\item 25.301 \cite{3gpp.25.301} Radio interface protocol architecture
	\item 25.323 \cite{3gpp.25.323} \gls{PDCP}: encapsulates \gls{RAB}
	\item 25.331 \cite{3gpp.25.331} RRC protocol specification (defined with \gls{ASN.1})
	\item 25.401 \cite{3gpp.25.401} UTRAN overall description
	\item 25.931 \cite{3gpp.25.931} UTRAN functions, examples on signalling procedures
	...
	\item 23.401 \cite{3gpp.23.401} \gls{E-UTRAN} procedures (LTE only)
	\item 23.402 \cite{3gpp.23.402} (LTE only) non-\gls{3GPP} accesses
	\item 24.007 \cite{3gpp.24.007} radio interface signaling
	\item 24.301 \cite{3gpp.24.301} EPS Non-Access-Stratum protocol between UE and MME on Uu
	\item 29.002 \cite{3gpp.29.002} Mobile Application Part (MAP) specification; part of SS7
