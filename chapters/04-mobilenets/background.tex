...
%Typically, these are mobility related (e.g. Routeing Area updates and handovers, a change in \gls{RAT}). 
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.8\textwidth]{images/rrc-state-model.pdf}
...
\gls{MM} models, specific only to the \gls{PS} connection of the device, are different for every \gls{RAT}


\begin{figure}[htb]
	\centering
	\includegraphics[width=0.6\textwidth]{images/pdp-state-model.pdf}
	\caption{\gls{PDP} State Model \cite[Section~9]{3gpp.23.060}.}
	\label{c4:fig:pdpstatemodel}
\end{figure}

\cite[Section~9]{3gpp.23.060} in Figure~\ref{c4:fig:pdpstatemodel} defines \gls{PDP} states and state machine, interacts closely with the \gls{MM} state machine
