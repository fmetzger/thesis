...
Contrary to the popular scientific belief, that the Internet is unable to change and renew itself (so called Internet ``ossification'', compare, e.g., \cite{feldmann2010ossification}), protocols in the \gls{TCP}/\gls{IP} stack change all the time.
...
other approaches include $\mu$tp: buffer friendly
...
WebSocket \cite{rfc6455}
 ...
 Provides full-duplex client-server communication
	...
	Barebones protocol atop TCP (or TLS), not HTTP
...
SPDY / \gls{HTTP}/2.0 (multiplexing -> segmented streaming) http://www.chromium.org/spdy/spdy-protocol/spdy-protocol-draft3-1
...
Alternate Transport Protocols (DCCP\cite{rfc4340}, LEDBAT\cite{rfc6817}/$\mu$tp\cite{bt2010utp}, QUIC, SCTP, DTLS)

SPDY: An experimental protocol for a faster web \cite{google2011SPDYdef} and \cite{google2010SPDYwp} http://tools.ietf.org/html/draft-mbelshe-httpbis-spdy-00
...
End-to-end encryption and Authentication mechanisms (e.g.IPSec, DNSSEC, CurveCP) %(Daniel J. Bernstein)
...
TCP Fast Open https://tools.ietf.org/html/draft-ietf-tcpm-fastopen + TCP Snap Start http://tools.ietf.org/html/draft-agl-tls-snapstart-00
 IETF draft
...
Congestion Avoidance algorithm (Tahoe, (New Reno), Vegas, Compound, BIC, CUBIC; all w or w/o SACK)
