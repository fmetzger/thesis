...
\subsubsection{Benefits for Other Applications}
...
Web browsers with \gls{HTTP} traffic might also be a good recipient of cross-layer information. ...

\todo{decide if to include demo figure for http reorder or not}

Also directly involving the user itself are a second category of cross-layer interactions that are only available if there is a downward control flow through the broker to the radio layer protocols. This would require the additional presence of a user-space policy manager. The manager would offer the user a series of preferences and a configuration interface for a rule-based engine. With this the user could create complex policies such as: ``Do not handover to a stationary WiFi from \gls{3G} when moving faster than \SI{50}{\kilo\meter\per\hour}, only to in-vehicle WiFi if available.'' or ``Avoid any vertical handover, which would interrupt my service for a long time, while a VoIP call is running.''
