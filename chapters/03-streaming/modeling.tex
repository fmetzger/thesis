...
Most of the aforementioned only have quality degradation from a video encoding process or sometimes additionally through classical transmission channels in mind.  

For a dedicated video streaming quality metric, only the streaming process should be factored in, and not the encoding process itself. During streaming only a specific subset of quality degradations can occur. Lost or late packets can cause missing blocks in a frame or frames to be skipped completely. Initial thoughts on \gls{IPTV} have been given in \cite{ituG1080}.
