...
In the loss measurement series, uncorrelated uniformly distributed loss was added in both the uplink and the downlink direction. The loss was incrementally increased in $2\%$ steps up to a total additional loss of $14\%$. About $60\%$ of the individual experiments did not finish correctly, and the video was not completely transmitted. This was caused by the \gls{TCP} stack, which at some point terminated the connection after to many packets were lost back to back, and curl giving up after several retries.

\todo{update the upper loss limit as well as the aborted number of runs and other values}
...
Figure~\ref{c3:fig:eval-loss-stallingtime} shows the resulting relative stalling duration in the packet loss measurement series. Loss of up to $4\%$ seems to have no discernible impact on the streaming process. Anything beyond this point sees a large increase in the stalling duration. 
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{images/R-playbackemulation-loss.pdf}
    \caption{Playback model observations on additional packet loss. Relative stalling time. (Temporary placeholder)}
