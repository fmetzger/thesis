...
RTP is a \textit{push-based approach}, the server has full control of the streaming process. Control and information exchange is also out of band through \gls{RTSP} and \gls{RTCP}
Adaptivity: yes, but limited when multicasting, and overhead (on-the-fly encoding)
...
It is also be used in conjunction with \gls{IGMP} \cite{rfc4604} at the network layer to create and stream to multicast groups. However, this is usually only seen in closed networks (``walled gardens''), where the whole network infrastructure is owned by a single instance.
