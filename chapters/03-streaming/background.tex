...
\todo{Mention Bernhard's A1TV measurements?}

\gls{RTP} \cite{rfc3550} is always used in conjunction with its sister-protocol \gls{RTCP} and usually also employs \gls{RTSP} \cite{rfc2326}. According to literature, they are the classic approach to video streaming (for example compare \cite[p.~589ff]{kurose2008computer} and \cite[p.~426ff]{peterson2007computer}).
...
\paragraph{RTSP}
 ...
 However, the requirement of several open \gls{UDP} sockets does not work well in environments using middleboxes, .e.g, firewall or \gls{NAT} nodes, because of the difficulty to forward incoming \gls{UDP} packets to the destined host. Furthermore, it mandated additional software to be installed at the client. 
...
\paragraph{Interaction with Underlying Protocols}
RTP is independent of transport protocol, can be either UDP or TCP (or one of the more exotic transport variants, \gls{SCTP} or \gls{DCCP} come to mind).

It is also be used in conjunction with \gls{IGMP} \cite{rfc4604} at the network layer to create and stream to multicast groups. However, all intermediary routers need to play along for this to work. Therefore, it is usually only seen in closed networks (``walled gardens''), where the whole network infrastructure is owned by a single instance.

WebRTC

\paragraph{RTP Variants}
ZRTP, Secure RTP

Use \gls{RTP} and \gls{RTCP} without RTSP but instead with \gls{SDP} \cite{rfc2327} to discover and initiate multicast \gls{RTP} streams.
