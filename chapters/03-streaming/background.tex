...
One of the alternatives to achieve this using \gls{HTTP}-streaming this is an appropriately sized flow control receive window. Alternatively, the \gls{HTTP}-server can also manually throttle the download process. A third way, is to either partition the stream file into smaller even-spaced segments, that have to be requested independently, or use the aforementioned range requests on the stream's file. Through this, the receiving application can delay the request of new ranges or segments so that it matches a targeted bitrate over a longer timeframe. This will however also result in a very bursty transmission, a so-called ON-OFF pattern, and can cause undesireably interactions with \gls{TCP}'s flow and congestion control mechanisms. Figure~\ref{c3:fig:streamingtransfermodes} compares these mechanisms in terms of their transmission pattern.
\todo{reference to transmission pattern here? alternatively, put one of the old youtube throttling pictures here}

These mechanisms can be subsumed under the term \textit{Application Layer Flow Control}
...
%%
\todo{Übersichtstabelle über alle Protokolle und Klassifizierung davon}
%%
