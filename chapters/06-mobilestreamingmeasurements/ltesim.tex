...
%\todo{Another series with a changed retrieval method in Figures x and y shows that the strong correlation to the latency is now greatly reduced}
...
The results in terms of the buffer fill level are displayed in Figure~\ref{c6:fig:ltesim-mobility-plotbuffer-facets}. The handover events between the two \gls{eNB} are denoted as gray vertical lines. The control transfer usually last only for about \gls{40}{\milli\second}. From the figures it is immediately evident that the video buffer, and therefore the throughput of the video segments, already suffers seconds before the actual handover event. The playback stop threshold is reached a number of times right at the handover mark.
