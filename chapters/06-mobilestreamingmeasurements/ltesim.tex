...
A possible hypothesis are issues with the approach to requesting subsequent segments. As these need to be requested in a timely manner while other segments are still being transmitted, they could be requested to late and do not saturate the link anymore. However, this effect is somewhat contrary to the additional effect observed at link speeds of \SI{100}{\mega\bit\per\second} and above.

Remember: LTE 80-90 mbit max, increase in stalling at or above this value, LTE is black box for user ip stack here, LTE conceal congestion but does not want to drop data; server overwhelms lte/epc does not trigger tcp congestion avoidance and cant scale down correctly to path bottleneck due to mobility buffers involved (essentially a form of bufferbloat?), throughput reduced to ~5mbit?; could also be related to issues of the actual lte simulation network but implementation can not fully responsible for this
