...
\begin{frame}
	...
	\vspace{1em}
	\only<2->{Reasoning for GGSN as load bottleneck}
	\begin{itemize}[<2->]
...
\begin{frame}
	...
	\begin{itemize}
		\item Link load of control traffic insignificant
			...
			\end{itemize}
		\item Significant portion of devices (\SI{18}{\percent}) that causes signaling traffic without transmitting any user traffic (M2M devices or old feature phones?)
	...
	\only<2->{Further investigations for load characterization}
	\begin{itemize}[<2->]
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item ``Signalling storm'' reaches core signaling: many short tunnels for specific device types
		\item Still, also slightly dependent on time of day
...
\begin{frame}
	...
	\framesubtitle{Virtualized GGSN Queuing Model}
		...
		\begin{itemize}
			...
			\item[$\rightarrow$] $M(t)/G(t)/\|\overrightarrow{c}\|_1/0$ ($|\overrightarrow{c}\|_1$: total tunnel capacity of all $s$ VMs) % 1-norm des Kapazitätsvektors
		\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Evaluate blocking probability and VM usage based on tunnel capacity
	...
	\begin{itemize}
		...
		\item Use as a load prediction tool for operators
		...
		\begin{itemize}
			...
			\item Location of streaming control
				\begin{itemize}
					...
					\item Client-side: pull-based (HTTP)
				\end{itemize}
			\item Selection of transport and application protocols and their transmission patterns
			% \item Transport properties
			% 	\begin{itemize}
			% 		\item Reliability
			% 		\item Multiplexing/multicasting abilities
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Complex mobile signaling interacts with higher protocol layers %in unforeseen ways
