...
\begin{frame}
	...
	Traditionally defined as direct user traffic load

	Does not factor in the vast control plane and signaling
	...
	The load of any particular mobile core network node $\rho_{n}$ could then be defined as

	\begin{equation}
		\phantom{,}\rho_{n} = \max(\rho_{n,l}, \rho_{n,m}, \rho_{n,p})\text{,}
	\end{equation}

	with the memory load $\rho_{m}$ and processing load $\rho_{p}$. And the total load of the CN $\rho_{CN}$ as
