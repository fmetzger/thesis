...
p <- p + coord_cartesian(xlim = c(0.5, 1000000)) + geom_hline(yintercept=0.5) + annotation_logticks(sides="b")
...
ggsave("R-tunnel-duration-device-type.pdf", width=12, height=10, useDingbats=F)
...
ggsave("R-tunnel-duration-operating-system.pdf", width=12, height=10, useDingbats=F)
...
ggsave("R-duration-classification-density.pdf", width=12, height=10, useDingbats=F)
embed_fonts("R-duration-classification-density.pdf")



## histogram attempts, not really working yet
histbreaks <- seq(0,max(servingtimes), max(servingtimes)/10000)

p <- ggplot(df, aes(x=st, color=as.factor(type)))
p <- p + geom_step(aes(st, y =..density..), stat = 'bin',breaks = histbreaks, position = "dodge")
p + scale_x_log10(limits=c(0.01, 1000000)) + scale_y_log10()
