...
p <- ggplot(dfsub, aes(x=max.tunnels,y=relative.blocking.probability,ymin=relative.blocking.probability.min,ymax=relative.blocking.probability.max))
p <- p + geom_point(stat = "identity") + geom_errorbar(width=1)
